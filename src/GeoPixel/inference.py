...
# if pred_masks:
    ...
    save_path = "{}/{}_masked.jpg".format(
        "vis_output", image_path.split("/")[-1]
        )
    # cv2.imwrite(save_path, save_img)
    # plt.close()
    # plt.imshow(save_img)
    # plt.title("\n".join(wrap(response, width=60)))
...
# with open("cog/GeoPixel/output.1.png", "rb") as image_file:
#     data = base64.b64encode(image_file.read())

# im = Image.open(BytesIO(base64.b64decode(data)))
# im.save('cog/GeoPixel/outputbase.0.png', 'PNG')

# out_np = base64.b64encode(np.asarray(im))
# im = Image.open(BytesIO(base64.b64decode(data)))
# im.save('cog/GeoPixel/outputbase.np.0.png', 'PNG')

# import requests

# # image = Image.open("cog/GeoPixel/output.1.png")
# image = Image.open("images/airport.jpg")
...
# # data = base64.b64encode(np.asarray(image))


# url = 'http://localhost:8070/predictions'
# payload = f'{{"input": {{"image": "data:image/png;base64,{data.decode("utf-8")}"}}}}'
# headers = {'content-type': 'application/json', 'Accept-Charset': 'UTF-8'}
# results = requests.post(url, data=payload, headers=headers)


# parsed_data = results.json()

# img_base64 = parsed_data["output"].split(";base64,", 1)[1]
...
# # Docker
# # subprocess.call()
# # os.system("docker inspect -f '{{.State.Status}}' focused_tesl")
# import subprocess
# import time
# import sys
# import os
# import signal # Useful for sending specific signals if needed

# # --- Configuration ---
# # 1. Define the command to start your server.
# #    Adjust this based on your specific server application.

# # Example 1: Python's built-in HTTP server
# # Runs 'python -m http.server 8080'
# server_command = ['cog', 'serve', '-p','8070']
...
#         # On Windows, you might need creationflags for better termination control
#         # creationflags=subprocess.CREATE_NEW_PROCESS_GROUP, # Example
#         # On Unix, preexec_fn can be used to set process group ID
#         # preexec_fn=os.setsid # Example: Makes it session leader, easier to kill group later
#     )
# # %%
